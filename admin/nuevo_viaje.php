...
$stmt = $pdo->prepare('SELECT * FROM remiseros WHERE activo = 1 AND rol = "remisero" AND tenant_id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$id_remisero) {
        ...
        if ($nuevo_pasajero) {
            ...
            $stmt->execute([$tenantId, $parts[0] ?? '', $parts[1] ?? '', $_POST['telefono'] ?? '', $_POST['direccion'] ?? '']);
