...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'crear') {
        $stmt = $pdo->prepare("INSERT INTO pasajeros (tenant_id, apellido, nombre, telefono, direccion) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$tenantId, $_POST['apellido'], $_POST['nombre'], $_POST['telefono'], $_POST['direccion']]);
        $message = 'Pasajero creado exitosamente';
    } elseif ($action === 'editar') {
        $stmt = $pdo->prepare("UPDATE pasajeros SET apellido = ?, nombre = ?, telefono = ?, direccion = ? WHERE id = ? AND tenant_id = ?");
        $stmt->execute([$_POST['apellido'], $_POST['nombre'], $_POST['telefono'], $_POST['direccion'], $_POST['id'], $tenantId]);
        $message = 'Pasajero actualizado';
    } elseif ($action === 'eliminar') {
        $stmt = $pdo->prepare("DELETE FROM pasajeros WHERE id = ? AND tenant_id = ?");
        $stmt->execute([$_POST['id'], $tenantId]);
        $message = 'Pasajero eliminado';
    }
}

$stmt = $pdo->prepare("SELECT * FROM pasajeros WHERE tenant_id = ? ORDER BY apellido, nombre");
