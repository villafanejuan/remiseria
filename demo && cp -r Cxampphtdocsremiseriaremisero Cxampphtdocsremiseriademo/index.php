...
$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM remiseros WHERE rol = 'remisero' AND activo = 1 AND tenant_id = ?");
$stmt->execute([$tenantId]);
$totalRemiseros = $stmt->fetch()['total'];

$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM pasajeros WHERE tenant_id = ?");
$stmt->execute([$tenantId]);
$totalPasajeros = $stmt->fetch()['total'];

$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM viajes WHERE DATE(created_at) = CURDATE() AND tenant_id = ?");
$stmt->execute([$tenantId]);
$viajesHoy = $stmt->fetch()['total'];

$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM viajes WHERE estado = 'buscando' AND tenant_id = ?");
$stmt->execute([$tenantId]);
